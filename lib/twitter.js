...
var JSONPAYLOAD_PATHS = [
  ...
  /tweets\/search\/(30day|fullarchive)\/(\w)/,
];

function checkPath(path, paths) {
  for(const p of paths) {
    if (path.search(p) !== -1) return true;
  }
  return false;
}
...
Twitter.prototype._buildReqOpts = function (method, path, params, isStreaming, callback) {
  ...
  if (checkPath(path, JSONPAYLOAD_PATHS)) {
    try {
  ...
  if (isStreaming) {
    ...
    if (FORMDATA_PATHS.indexOf(path) !== -1) {
      ...
      // of the params
      finalParams = {};
    } else if (checkPath(path, JSONPAYLOAD_PATHS)) {
